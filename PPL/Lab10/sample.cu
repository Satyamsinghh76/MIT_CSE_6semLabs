...
An optimization technique that breaks large matrices into smaller sub-matrices, or "tiles," which fit into fast cache or shared memory. 
Reduces costly global memory accesses by reusing data, enabling significant speedups on CPUs and especially GPUs
...
// Grid = (4/2, 4/2) = 2 × 2 blocks => Each block handles a 2×2 portion of output
#define BLOCK_WIDTH 2
#define TILE_WIDTH 2
#define WIDTH 4

__global__ void MatMulElementThreadShared(int *a, int *b, int *c) {
    __shared__ int MDs[TILE_WIDTH][TILE_WIDTH]; //Mat A 's tile
    __shared__ int NDs[TILE_WIDTH][TILE_WIDTH]; //Mat B 's tile
